export const about = {
	en: {
		'about.title': 'Jo Maendle',
		'about.subtitle': 'A Bremen-based photographer specializing in nature and portraits.',
		'about.desc.1':
			'With over 9 years of experience in capturing the beauty of nature and landscapes, my work has garnered hundreds of millions of views worldwide on ',
		'about.desc.2':
			"Since the start of 2024, I've expanded my focus to include outdoor portrait photography.",
		'about.desc.3':
			'My passion lies in encapsulating the unique beauty of individuals and their stories. I strive to create authentic and timeless images that will be treasured for a lifetime.',
		'about.cta.title': "Let's work together",
		'about.cta.subtitle': 'Ready to book a session or have questions? Contact us today.',
		'about.cta.offers':
			'I offer outdoor portrait sessions for individuals and couples in Bremen and surrounding areas.',
		'about.unsplash.views': 'Views',
		'about.unsplash.downloads': 'Downloads',
		'about.unsplash.totalPhotos': 'Photos',
		'about.unsplash.followers': 'Followers',
		'about.unsplash.title': 'Unsplash Stats',
	},
	de: {
		'about.title': 'Jo Maendle',
		'about.subtitle':
			'Ein in Bremen ansässiger Fotograf, spezialisiert auf Natur- und Porträtaufnahmen.',
		'about.desc.1':
			'Mit über 9 Jahren Erfahrung im Einfangen der Schönheit von Natur und Landschaften hat meine Arbeit weltweit bereits hunderte Millionen Aufrufe auf ',
		'about.desc.2':
			'Seit Anfang 2024 habe ich meine Arbeit um Outdoor-Porträtfotografie erweitert.',
		'about.desc.3':
			'Meine Leidenschaft liegt darin, die einzigartige Schönheit von Individuen und ihren Geschichten einzufangen. Ich bemühe mich, authentische und zeitlose Bilder zu schaffen, die ein Leben lang geschätzt werden.',
		'about.cta.title': 'Lass uns zusammenarbeiten',
		'about.cta.subtitle':
			'Bereit für eine Buchung oder hast du Fragen? Kontaktiere mich noch heute.',
		'about.cta.offers':
			'Ich biete Outdoor-Porträtsessions für Einzelpersonen und Paare in Bremen und Umgebung an.',
		'about.unsplash.views': 'Impressionen',
		'about.unsplash.downloads': 'Downloads',
		'about.unsplash.totalPhotos': 'Fotos',
		'about.unsplash.followers': 'Follower',
		'about.unsplash.title': 'Unsplash Statistiken',
	},
};
